...
def generate_launch_description():
    ...
    """
    avp_demo_pkg_prefix = get_package_share_directory('trajectory_follower_nodes')
    
    vehicle_characteristics_param_file = os.path.join(
        avp_demo_pkg_prefix, 'param/vehicle_characteristics.param.yaml')
    lat_control_param_file = os.path.join(
        avp_demo_pkg_prefix, 'param/lateral_controller.param.yaml')
    lon_control_param_file = os.path.join(
        avp_demo_pkg_prefix, 'param/longitudinal_controller.param.yaml')
    latlon_muxer_param_file = os.path.join(
        avp_demo_pkg_prefix, 'param/latlon_muxer.param.yaml')
    ...
    lat_control = Node(
        ...
        remappings=[
           ("input/reference_trajectory", "/planning/scenario_planning/lane_driving/trajectory"),#/planning/scenario_planning/lane_driving/trajectory
    ...
    lon_control = Node(
        ...
        remappings=[
           ("input/current_trajectory", "/planning/scenario_planning/lane_driving/trajectory"),#/planning/scenario_planning/lane_driving/trajectory
