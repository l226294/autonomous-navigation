  ...
  const ControlData & control_data)
{
  ...
  if (current_control_state == ControlState::DRIVE) {
    ...
    RCLCPP_INFO(
      get_logger(),
      "[feedback control]  vel: %3.3f km/h, acc: %3.3f, dt: %3.3f, v_curr: %3.3f km/h, v_ref: %3.3f km/h, "
      "feedback_ctrl_cmd.ac: %3.3f",
      raw_ctrl_cmd.vel * 3.6, raw_ctrl_cmd.acc, control_data.dt, current_vel * 3.6, target_motion.vel * 3.6,
      raw_ctrl_cmd.acc);
  } else if (current_control_state == ControlState::STOPPING) {
