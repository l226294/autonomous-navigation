...
: Node("Lanelet2MapProvider", options)
{
  ...
  RCLCPP_INFO(get_logger(), map_filename.c_str());
  ...
  if (has_parameter("latitude") && has_parameter("longitude") && has_parameter("elevation")) {
    ...
    RCLCPP_INFO(get_logger(), "auto_map_provider");
  } else {
    ...
    // const float64_t origin_lat = 39.95904886799;
    // const float64_t origin_lon = 116.30503669314;
    // const float64_t origin_alt = 0.0;
    // LatLonAlt map_origin{origin_lat, origin_lon, origin_alt};

    // RCLCPP_INFO(get_logger(), "m_map_provider");
    // // "/home/luo/install/autoware_demos/share/autoware_demos/data/autonomoustuff_parking_lot.osm"
    // m_map_provider = std::make_unique<Lanelet2MapProvider>(
    //   map_filename, map_origin, origin_offset_lat, origin_offset_lon);
    RCLCPP_INFO(get_logger(), "m_map_provider");
  }
  const auto map_bin_msg = create_map_bin_msg();

  pub_map_bin_ =
    create_publisher<autoware_auto_mapping_msgs::msg::HADMapBin>("output/lanelet2_map", rclcpp::QoS{1}.transient_local());
...
autoware_auto_mapping_msgs::msg::HADMapBin Lanelet2MapProviderNode::create_map_bin_msg()
{
  autoware_auto_mapping_msgs::msg::HADMapBin msg;
  msg.header.frame_id = "map";
