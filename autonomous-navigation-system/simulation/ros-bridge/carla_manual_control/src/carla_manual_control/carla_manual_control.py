...
class KeyboardControl(object):
    ...
    def __init__(self, role_name, hud, node):
        ...
        self._other_actors_autopilot_enabled = False
        self._spectate_ego_vehicle = True
        ...
        self.other_actors_auto_pilot_enable_publisher = self.node.new_publisher(
            Bool,
            "/carla/enable_npc_autopilot",
            qos_profile=fast_qos)
        
        self.shift_view_publisher = self.node.new_publisher(
            Bool, 
            "/carla/shift_view",
            qos_profile=fast_qos)

        self.spawn_new_objects_publisher = self.node.new_publisher(
            Bool, 
            "/carla/spawn_new_objects",
            qos_profile=fast_qos)      
        ...
        self.set_other_actors_autopilot(self._other_actors_autopilot_enabled)
    ...
    def set_other_actors_autopilot(self, enable):
        """
        enable/disable the autopilot
        """
        self.other_actors_auto_pilot_enable_publisher.publish(Bool(data=enable))
    
    def spectate_ego_vehicle(self, enable):
        self.shift_view_publisher.publish(Bool(data=enable))
    
    def spawn_new_objects(self, enable):
        self.spawn_new_objects_publisher.publish(Bool(data=enable))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == K_q:
                    ...
                    self._other_actors_autopilot_enabled = not self._other_actors_autopilot_enabled
                    self.set_other_actors_autopilot(self._other_actors_autopilot_enabled)
                    self.hud.notification('npc Autopilot %s' %
                                          ('On' if self._other_actors_autopilot_enabled else 'Off'))
                elif event.key == K_v:
                    self._spectate_ego_vehicle = not self._spectate_ego_vehicle
                    self.spectate_ego_vehicle(self._spectate_ego_vehicle)
                elif event.key == K_n:
                    self.spawn_new_objects(True)
...
class HUD(object):
    ...
    def update_info_text(self):
        ...
        accel = self.vehicle_status.acceleration.linear.x
