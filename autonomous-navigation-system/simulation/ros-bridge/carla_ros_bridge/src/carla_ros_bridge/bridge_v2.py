...
class CarlaRosBridge(CompatibleNode):
    ...
    def initialize_bridge(self, carla_client, carla_world, params):
        """
        ...
        self.traffic_manager.set_random_device_seed(9)
        self.carla_world = carla_world
        self.num_of_new_actors_created_manually = 0

        self.view_spectate_ego_vehicle = True
        ...
        self.set_other_actors_autopilot = \
            self.new_subscription(Bool, "/carla/enable_npc_autopilot",
                                  self.enable_npc_autopilot, qos_profile=10, callback_group=self.callback_group)
        
        self.shift_view = \
            self.new_subscription(Bool, "/carla/shift_view",
                                  self.shift_view, qos_profile=10, callback_group=self.callback_group)
        
        self.spawn_new_objects_sub = \
            self.new_subscription(Bool, "/carla/spawn_new_objects", 
                                  self.spawn_new_objects, qos_profile=10, callback_group=self.callback_group)

    def spawn_new_objects(self, enable):
        req = roscomp.get_service_request(SpawnObject)
        req.type = "vehicle.*"
        # req.type = "walker.*"
        req.attach_to = 0
        req.random_pose = True
        for i in range(10):
            self.num_of_new_actors_created_manually += 1
            req.id = "autopilot" + str(self.num_of_new_actors_created_manually)
            id_ = self.actor_factory.spawn_actor(req, enable.data)
    ...
    def enable_npc_autopilot(self, enable_auto_pilot):
        vehicles_list = self.carla_world.get_actors().filter('vehicle.*')
        # self.traffic_manager.global_percentage_speed_difference(-50)
        for vehicle in vehicles_list:
            if "role_name" in vehicle.attributes and vehicle.attributes["role_name"] == "ego_vehicle":
                continue
            vehicle.set_autopilot(enable_auto_pilot.data, 8000)
            # self.traffic_manager.auto_lane_change(vehicle, True)            
            self.traffic_manager.ignore_lights_percentage(vehicle, 100)
    
    def shift_view(self, enable_shift_view):
        self.view_spectate_ego_vehicle = enable_shift_view.data
    ...
    def _synchronous_mode_update(self):
        ...
        while not self.shutdown.is_set() and roscomp.ok():
            ...
            if self.actor_factory.ego_vehicle_id is not None and self.view_spectate_ego_vehicle:
                spectator = self.carla_world.get_spectator()
                ego_vehicle = self.carla_world.get_actor(self.actor_factory.ego_vehicle_id)
...
def main(args=None):
    ...
    try:
        ...
        carla_bridge.initialize_bridge(carla_client, carla_client.get_world(), parameters)
