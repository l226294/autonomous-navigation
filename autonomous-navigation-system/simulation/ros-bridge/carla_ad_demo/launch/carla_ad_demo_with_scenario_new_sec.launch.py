...
ros_topic_msg_string = "{{ 'scenarios': [{{ 'name': 'FollowLeadingVehicle', 'scenario_file': '{}'}}] }}".format(
    os.path.join(get_package_share_directory('carla_ad_demo'), 'config/FollowLeadingVehicle_1.xosc'))
...
def generate_launch_description():
    ld = launch.LaunchDescription([
        ...
        launch.actions.DeclareLaunchArgument(
            ...
            default_value='/home/luo/scenario_runner'
        ),
        launch.actions.DeclareLaunchArgument(
            name='role_name',
        ...
        launch.actions.IncludeLaunchDescription(
            launch.launch_description_sources.PythonLaunchDescriptionSource(
                os.path.join(get_package_share_directory(
                    'carla_spawn_objects'), 'carla_example_ego_vehicle_v2.launch.py')
            ),
            launch_arguments={
                'object_definition_file': get_package_share_directory('carla_spawn_objects') + '/config/objects_v2.json',
